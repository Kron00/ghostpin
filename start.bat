@echo off
:: UNMAINTAINED. Ghostpin is developed and tested on macOS only. This script is
:: kept for anyone who wants to pick Windows support back up, but it is known to
:: be incomplete: the tunneld fallback below is launched without elevation, so it
:: will fail on a non-administrator account. See the README's Windows section.
setlocal enabledelayedexpansion

echo ==================================================
echo   Ghostpin (Windows Dev Mode - unmaintained)
echo ==================================================
echo.
...
start /b "" %VPYTHON% -m pymobiledevice3 remote tunneld > %TEMP%\ghostpin-tunneld.log 2>&1
...
if %STARTED%==0 (
    ...
    type %TEMP%\ghostpin-tunneld.log 2>nul
    pause
    exit /b 1
)
